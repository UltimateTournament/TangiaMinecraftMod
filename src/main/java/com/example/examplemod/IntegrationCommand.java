...
public class IntegrationCommand {
  public IntegrationCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
    dispatcher.register(Commands.literal("tangia").then(Commands.literal("set").then(Commands.argument("testarg", StringArgumentType.string())).executes((command) -> {
      return integrate(command);
    })));
  }
